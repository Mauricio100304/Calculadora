...
class MainActivity : AppCompatActivity() {
    ...
    fun calcular (view : View){
        ...
        var tboton=boton.text.toString()
        var con=textView0?.text.toString()+tboton
        var mostrar=qceros(con)
        if(tboton=="="){
            var resultado=0.0
            try {
                resultado=eval(textView0?.text.toString())
                textView0?.text=resultado.toString()
            }catch (e:Exception){
                textView0?.text=e.toString()
            }
        }else if (tboton=="C"){
            textView0?.text="0"
        }else{
            textView0?.text=mostrar
        }

    }
    fun qceros(str : String):String{
        var i=0
